...
function renderBonusAlphabet() {
  ...
  for (const letter in bonusAlphabet) {
    const letterFieldElt = $make("div", $(".bonusAlphabetField"), { className: "letterField" });
    $make("label", letterFieldElt, { textContent: letter.toUpperCase() })
    const letterInput = $make("input", letterFieldElt, { type: 'number', min: '0', id: letter, value: bonusAlphabet[letter] })
...
function socket_onSetup(data) {
  ...
  $(".resetBonusAlphabet").addEventListener("click", (e) => {
    ...
    for (const letterFieldInput of $$('.letterField input'))
      letterFieldInput.value = milestone.dictionaryManifest.bonusAlphabet[letterFieldInput.id];
  ...
  $(".saveBonusAlphabet").addEventListener("click", (e) => {
    ...
    for (const { id, value } of $$('.letterField input')) customBonusAlphabet[id] = parseInt(value, 10);
  ...
  $(".bonusAlphabetBulkEdit button").addEventListener("click", (e) => {
    ...
    for (const letterFieldInput of $$('.letterField input')) letterFieldInput.value = parseInt(bulkEditValue, 10);
    isBonusAlphabetEdited = true;
    $show($(".resetBonusAlphabet"), isBonusAlphabetEdited);
    $show($(".saveBonusAlphabet"), isBonusAlphabetEdited);
  })
...
const maxPlayersRuleField = { number: $(".rules .maxPlayers.rule input[type=number]"), range: $(".rules .maxPlayers.rule input[type=range]") };
...
$bindRangeField(maxPlayersRuleField, () => socket.emit("setRules", { maxPlayers: parseInt(maxPlayersRuleField.number.value, 10) }));
...
function socket_onSetRules(data) {
  ...
  for (const [ruleName, value] of Object.entries(data)) {
    ...
    switch (ruleName) {
      ...
      case "customBonusAlphabet": renderBonusAlphabet(); break;
    }
  }
}
...
function round_enter() {
  ...
  wordInput.value = "";
  if (document.getElementById("hintHolder") !== null) socket.emit("hintHolder", document.getElementById("hintHolder").textContent);
}
...
function animate(animateTime) {
  ...
  if (milestone.name === "round") {
    ...
    if (selfPlayerState != null && radius > 50) {
      ...
      for (const letter in customBonusAlphabet) {
        ...
        ctx.fillStyle = selfPlayerState.bonusLetters[letter] <= 0 ? "#888" : "#db6";
        ...
        ctx.restore();
        if (customBonusAlphabet[letter] > 0 && selfPlayerState.bonusLetters[letter] > 1) {
          ctx.font = "bold 0.7em 'Lato'";
          ctx.fillStyle = "#444";
          ctx.fillText(selfPlayerState.bonusLetters[letter], horizontal ? offset1 - letterSize / 3.5 : offset2 - letterSize / 3.5, horizontal ? offset2 + letterSize * 2 / 6 : offset1 + letterSize * 2 / 5.5);
        }
  ...
  function drawPlayerAvatars() {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (player.image.src.length && player.image.complete && player.image.naturalHeight !== 0) {
